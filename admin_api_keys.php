...
define('ENABLE_ADMIN_DEBUG', false);

// 0. Path Detection & Autoload
// Support running from root or subfolder (e.g. /PostMoon/)
$possible_autoloads = [
    __DIR__ . '/../common/autoload.php', // Standard Rhymix in parent
    __DIR__ . '/common/autoload.php',    // If in root
    $_SERVER['DOCUMENT_ROOT'] . '/common/autoload.php',
    $_SERVER['DOCUMENT_ROOT'] . '/rhymix/common/autoload.php'
];

$autoload_path = null;
foreach ($possible_autoloads as $path) {
    if (file_exists($path)) {
        $autoload_path = $path;
        break;
    }
...
if (!$autoload_path) {
    die("<h1>System Error</h1><p>Rhymix Core not found. (autoload.php missing)</p><p>Searched paths:<br>" . implode('<br>', $possible_autoloads) . "</p>");
}

require_once $autoload_path;

// 1. Fix CWD (Current Working Directory) for Rhymix
// Rhymix expects to run from the root directory.
$rhymix_root = dirname($autoload_path, 2); // ../common/.. -> root
if (is_dir($rhymix_root)) {
    chdir($rhymix_root);
}
...
function debug_log($msg) {
    if (defined('ENABLE_ADMIN_DEBUG') && ENABLE_ADMIN_DEBUG) {
        // Write to PostMoon folder, not CWD (which is now root)
        $log_file = __DIR__ . '/admin_debug_log.txt';
