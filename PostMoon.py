...
class PostMoonApp:
    def __init__(self, root):
        self.root = root
        self.VERSION = "v1.5.12 (Fix: Config Path)" # Updated
        self.root.title(f"PostMoon - AI Powered Rhymix Uploader {self.VERSION}")
        self.root.geometry("1200x800") # Reduced height to fit on standard screens
        ...
        # [Reordered] 1. Action Section (Send Button) - Pack Bottom First
        ...
        self.send_btn.pack(side="bottom", fill="x", pady=(10, 0))

        # [Reordered] 2. File Upload Section - Pack Bottom Above Button
        file_frame = tk.Frame(self.right_frame)
        file_frame.pack(side="bottom", fill="x", pady=(0, 0))
        ...
        # [Reordered] 3. Right Header - Pack Top
        tk.Label(self.right_frame, text="🚀 결과 확인 및 전송 (Result)", font=("Arial", 14, "bold")).pack(side="top", anchor="w", pady=(0, 10))

        # [Reordered] 4. Settings Summary & Toggle - Pack Top
        settings_summary_frame = tk.Frame(self.right_frame)
        settings_summary_frame.pack(side="top", fill="x", pady=(0, 5))
        ...
        # [Reordered] 5. Post Content Section - Pack Top (Fill Remaining)
        self.content_frame = tk.LabelFrame(self.right_frame, text="게시글 내용", padx=10, pady=5)
        self.content_frame.pack(side="top", fill="both", expand=True, pady=(0, 10))
    ...
    def toggle_settings(self):
        if self.config_wrapper.winfo_ismapped():
            ...
            self.config_wrapper.pack(side="top", fill="x", pady=(0, 5), before=self.content_frame)
    ...
    def load_config(self):
        ...
        try:
            ...
            # [Legacy Support] Check if config.json exists in current dir (migration)
                 ...
                 try:
                    with open('config.json', 'r', encoding='utf-8') as f:
                        ...
                        self.profiles = data.get('profiles', {})
                        self.current_profile = data.get('last_used', 'Default')
                        # We will save to new location on exit/save
                 except:
                     pass
        ...
        # [NEW] Bind context menu to all text widgets after UI is built
        self.bind_context_menu(self.root)

    def bind_context_menu(self, widget):
        # Recursively bind right-click menu to Text and Entry widgets
        try:
            ...
            if isinstance(widget, scrolledtext.ScrolledText):
                # Access the internal Text widget
                widget.text.bind("<Button-3>", self.show_context_menu)
        except Exception:
        ...
        try:
            ...
            try:
                if 'response' in locals() and response and hasattr(response, 'text'):
                     debug_info = f"\n\n[Debug] 서버 응답 내용 (첫 300자):\n{response.text[:300]}"
            except: pass
            
            messagebox.showerror("실패", f"전송 실패: {str(e)}{debug_info}")
        finally:
            # Clean up files
            for f in open_files:
                f.close()
