...
class PostMoonApp:
    def __init__(self, root):
        self.root = root
        self.VERSION = "v1.6.2 (KUTF Style Added)"
    ...
    def update_summary_label(self):
        ...
        if url:
            try:
                ...
                domain = url
        if not mid: mid = "mid 미설정"
        self.current_target_label.configure(text=f"현재 타겟: {domain} / {mid}")
    ...
    def upload_to_rhymix(self):
        ...
        if not all([api_url, api_key, mid, title, content]):
            return messagebox.showwarning("입력 오류", "모든 필드를 입력해주세요.")
