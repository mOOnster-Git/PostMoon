...
debug_log("Script Version: 1.5.18");
...
// [V1.5.9] Fix: Change Current Working Directory to Rhymix Root
// This is critical when running from a subfolder (PostMoon/)
// Rhymix expects to run from the root directory for file paths to work correctly.
$rhymix_root = dirname(dirname($autoload_path)); // common/autoload.php -> root
chdir($rhymix_root);
...
try {
    ...
    if (method_exists($oDB, 'escapeString')) {
        ...
        $safe_key = addslashes($api_key); // Fallback
    }
    ...
    // [V1.5.18] Simplified query check logic (admin_api_keys.php와 동일 방식)
    $row = null;
    if ($stmt) {
         $row = $oDB->fetch($stmt);
    } else {
...
if (!$row) {
    debug_log("Key not found in DB. (File auth removed in v1.5.17)");
